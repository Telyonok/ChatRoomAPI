...
namespace ChatRoomAPI.Helpers
{
    public static class Crypto
    {
        ...
        private const string Crypto_NotSupportedHashAlg = "The hash algorithm {0} is not supported, valid values are: sha256, sha1, md5";
        ...
        public static string Hash(byte[] input, string algorithm = "sha256")
        {
            ...
            using (HashAlgorithm alg = HashAlgorithm.Create(algorithm))
            {
                ...
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, Crypto_NotSupportedHashAlg, algorithm));
                }
            }
        }
